...
pub fn ensure_ffmpeg_binary() {
    ...
    match download_and_extract_ffmpeg(&target, &binary_path) {
        Ok(()) => {
            ...
            if !verify_ffmpeg_binary(&binary_path) {
                println!(
                    "cargo:warning=⚠️  Downloaded FFmpeg binary verification failed, continuing without bundled FFmpeg"
                );
            }
        }
        Err(e) => {
            println!(
                "cargo:warning=⚠️  Failed to download FFmpeg: {}. Continuing without bundled FFmpeg.",
                e
            );
        }
    }
